...
try:
    from scripts.ecommerce_relevance import add_ecommerce_relevance_fields
except ModuleNotFoundError:  # pragma: no cover - direct `python scripts/update_news.py`
    from ecommerce_relevance import add_ecommerce_relevance_fields
...
def collect_all(session: requests.Session, now: datetime, topic: str = "ai") -> tuple[list[RawItem], list[dict[str, Any]]]:
    tasks = []
    if topic == "ai":
        tasks = [
...
def fetch_opml_rss(
    ...
    if resolved_feeds:
        worker_count = min(4, max(1, len(resolved_feeds)))
        with ThreadPoolExecutor(max_workers=worker_count) as executor:
...
def ecommerce_impact_for_story(item: dict[str, Any], channel: str) -> str:
    # Prefer the richer per-item impact generated by ecommerce_relevance scoring
    if item.get("impact"):
        return str(item["impact"])
    if item.get("business_value"):
        return str(item["business_value"])
    return {
...
def ecommerce_action_for_story(item: dict[str, Any], channel: str) -> str:
    # Prefer the richer per-item suggested_action
    if item.get("suggested_action"):
        return str(item["suggested_action"])
    return {
...
def build_story_record(
    ...
    return {
        ...
        "primary_item": {
            ...
            "ai_label": primary.get("ai_label") or brief_channel,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate topic news updates from multiple sources")
    ...
    parser.add_argument(
        ...
        help="Topic relevance profile to apply",
    )
